...
Author: ChatGPT (for Carlos Michael Mejia Granda)
Date: 2025-12-18
...
4) has_os_time_source populated from SOURCE_CODE_OPENMRS evidence (prioritizes VisitRepository.java / System.currentTimeMillis()).
5) CRITICAL: Correct primary AndroidManifest.xml selection using deterministic content scoring (prefers manifest with <application>, services, activities, openmrs-client).
...
openmrs_zip_path = "/mnt/data/openMRS.zip"
openmrs_texts = read_all_text_files_from_zip(openmrs_zip_path)

combined_code = "\n".join(openmrs_texts.values())
code_lower = combined_code.lower()

readme_text = ""
for path, content in openmrs_texts.items():
...
def find_source_manifests(openmrs_texts_obj):
    candidates = []
    for path in openmrs_texts_obj:
        if path.lower().endswith("androidmanifest.xml"):
            candidates.append(path)
    return candidates


def sort_manifests_by_content(manifest_paths, openmrs_texts_obj):
    """
    Deterministic primary manifest selection by content:
      +1000 if contains <application
      +50 per <service
      +20 per <activity
      +200 if path contains openmrs-client
    ...
    for p in (manifest_paths or []):
        txt = (openmrs_texts_obj or {}).get(p) or ""
        low = txt.lower()
        score = 0
...
source_manifest_paths = sort_manifests_by_content(find_source_manifests(openmrs_texts), openmrs_texts)
source_manifest_text = openmrs_texts.get(source_manifest_paths[0]) if source_manifest_paths else None
...
PREFERRED_TIME_EVIDENCE = [
    {
        "path_suffix": "/openmrs-android-sdk/src/main/java/com/openmrs/android_sdk/library/api/repository/visitrepository.java",
...
def detect_os_time_source_evidence(openmrs_texts_obj, max_evidence=5):
    evidence = []
    hit_paths = []
    items = list((openmrs_texts_obj or {}).items())
    ...
    for path in scan_paths:
        txt = (openmrs_texts_obj or {}).get(path) or ""
...
os_time_source_evidence = detect_os_time_source_evidence(openmrs_texts)
...
def detect_password_hashing_features(openmrs_texts_obj, max_evidence=8):
    evidence = []
    hit_paths = set()
    kdf_algs = set()
    salt_hits = False
    ...
    preferred_suffix = "/openmrs-android-sdk/src/main/java/com/openmrs/android_sdk/library/openmrsandroid.java"
    preferred_paths = []
    for p in (openmrs_texts_obj or {}).keys():
        if p.replace("\\", "/").lower().endswith(preferred_suffix):
            preferred_paths.append(p)

    scan_paths = preferred_paths + [p for p in (openmrs_texts_obj or {}).keys() if p not in preferred_paths]
    ...
    for path in scan_paths:
        ...
        for alg, pat in PASSWORD_HASHING_KDF_PATTERNS:
            ...
            if m:
                ...
                hit_paths.add(path)
                excerpt = _extract_line_excerpt(txt, m.start())
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{path}", f"password_kdf_{alg}", excerpt))
                break
        ...
        for alg, pat in PASSWORD_HASHING_SALT_PATTERNS:
            ...
            if m:
                ...
                hit_paths.add(path)
                excerpt = _extract_line_excerpt(txt, m.start())
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{path}", f"password_salt_{alg}", excerpt))
                break
...
password_hashing_features = detect_password_hashing_features(openmrs_texts)

# ============================================================
# Extra features: Gradle, WebView, vulnerabilities, Manifest
# ============================================================

def detect_keystore_env_paths(openmrs_texts_obj):
    env_paths = []
    for path, txt in openmrs_texts_obj.items():
...
def detect_signing_creds_hardcoded(openmrs_texts_obj):
    for path, txt in openmrs_texts_obj.items():
        lower_path = path.lower()
        if not (lower_path.endswith("build.gradle") or lower_path.endswith(".gradle")):
            continue
...
def detect_release_minify_disabled(openmrs_texts_obj):
    for path, txt in openmrs_texts_obj.items():
...
def detect_manifest_debuggable_true(source_manifest_text, mobsf_static_obj):
    ...
    if source_manifest_text and source_manifest_paths:
        src_ev.append(ev(
            "SOURCE_CODE_OPENMRS",
            f"openMRS.zip:{source_manifest_paths[0]}",
            "android:debuggable",
    ...
    for i, line in enumerate(low.splitlines()):
        ...
        if 'android:exported="true"' in l:
            if 'permission=' not in l and 'android:permission=' not in l:
                ...
                evidence.append(ev(
                    "SOURCE_CODE_OPENMRS",
                    f"openMRS.zip:{source_manifest_paths[0]}:line{ i+1 }" if source_manifest_paths else "openMRS.zip:AndroidManifest.xml",
                    "android:exported",
                    line.strip()[:200]
                ))
...
def detect_manifest_custom_permissions(manifest_text: str) -> Dict[str, Any]:
    ...
    for idx, tag in enumerate(tags[:8]):
        ...
        evidence.append(ev(
            "SOURCE_CODE_OPENMRS",
            f"openMRS.zip:{source_manifest_paths[0]}:permission[{idx}]" if source_manifest_paths else "openMRS.zip:AndroidManifest.xml",
...
def detect_manifest_signature_level_defined(manifest_text: str) -> Dict[str, Any]:
    ...
    if not is_true and tags:
        ...
        evidence.append(ev(
            "SOURCE_CODE_OPENMRS",
            f"openMRS.zip:{source_manifest_paths[0]}:permission[0]" if source_manifest_paths else "openMRS.zip:AndroidManifest.xml",
...
def detect_exported_broadcast_receivers_without_permission(manifest_text: str, manifest_path: str) -> Dict[str, Any]:
    ...
    for idx, m in enumerate(receiver_blocks, start=1):
        ...
        if insecure:
            ...
            evidence.append(ev(
                "SOURCE_CODE_OPENMRS",
                f"openMRS.zip:{manifest_path}:receiver[{idx}]",
    ...
    if insecure_count == 0:
        ...
        evidence.append(ev(
            "SOURCE_CODE_OPENMRS",
            f"openMRS.zip:{manifest_path}:receiver[{sample_idx}]",
...
def detect_manifest_services_explicit_accessibility_attributes(source_manifest_text, source_manifest_paths_obj=None):
    ...
    for idx, tag in enumerate(tags):
        ...
        if "android:exported" not in low:
            ...
            evidence.append(ev(
                "SOURCE_CODE_OPENMRS",
                f"openMRS.zip:{manifest_path}:service[{idx}]",
...
def detect_logout_and_session_features(openmrs_texts_obj):
    features = {
...
def detect_endpoint_auth_features(openmrs_texts_obj):
    features = {
...
logout_session_features = detect_logout_and_session_features(openmrs_texts)
endpoint_auth_features = detect_endpoint_auth_features(openmrs_texts)

# ============================================================
# WebView remote content (simple)
# ============================================================

def detect_webview_remote_content(openmrs_texts_obj):
    paths = []
...
webview_remote_paths = detect_webview_remote_content(openmrs_texts)
...
def detect_kill_switch_and_safe_mode():
    ...
    add_source("openmrs_code", code_lower)
...
def detect_temp_compiled_data_controls(openmrs_texts_obj):
    protection_paths = []
    secure_delete_paths = []
...
temp_compiled_features = detect_temp_compiled_data_controls(openmrs_texts)
...
for path, txt in openmrs_texts.items():
    if path.lower().endswith("build.gradle"):
        low = (txt or "").lower()
...
keystore_env_paths = detect_keystore_env_paths(openmrs_texts)
has_signing_creds_hardcoded = detect_signing_creds_hardcoded(openmrs_texts)
release_minify_disabled = detect_release_minify_disabled(openmrs_texts)
...
def detect_org_validates_audit_tool_integrity_periodically(openmrs_texts_obj, max_evidence=6):
    """
    ...
    for fpath, content in (openmrs_texts_obj or {}).items():
...
def compute_flag_verdict(
    ...
    if flag_id == "has_org_validates_audit_tool_integrity_periodically":
        strict = detect_org_validates_audit_tool_integrity_periodically(openmrs_texts)
    ...
    if flag_id in ORG_FLAGS or flag_id.startswith("has_org_") or flag_id.startswith("has_defined_"):
        ...
        if has_evidence:
            ...
            for src in evidence_sources:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"ORG_INDEX:{src}", "org_policy_reference", "Evidence found via ORG index"))
        else:
            yn = "NO"
            state = "fail"
            notes = (
    ...
    if flag_id == "has_supports_runtime_permission_management":
        ...
        if supports:
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:code_scan", "runtime_permissions_api", "requestPermissions/ActivityCompat/etc detected"))
    ...
    if flag_id == "has_hardcoded_credentials":
        ...
        if code_features_obj.get("has_hardcoded_password_literal"):
            ...
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:code_scan", "password_literal", 'password = "..." pattern'))
        if has_default_credentials_obj:
            notes_parts.append("The README documents demo credentials: admin / admin123.")
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:README.md", "default_credentials", "demo username/password documented"))
        if not notes_parts:
    ...
    if flag_id == "has_secrets_secure_keystore_env_vars":
        ...
        if paths:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "gradle_signing_env", "signingConfigs uses System.getenv"))
    ...
    if flag_id == "has_signing_creds_not_hardcoded":
        ...
        if has_env_keystore:
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:gradle", "signing_env", "System.getenv used for signing configs"))
        if has_hardcoded:
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:gradle", "signing_hardcoded", "storePassword/keyPassword literal detected"))
        return {"state": state, "summary": f"{flag_id} = {yn}", "notes": " ".join(notes_parts), "evidence": evidence}
    ...
    if flag_id == "has_secure_cicd_key_management":
        ...
        if env_paths:
            ...
            for p in env_paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "gradle_env", "System.getenv for signing configs"))
        else:
        ...
        if has_signing_hardcoded:
            ...
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:gradle", "gradle_hardcoded_secret", "storePassword/keyPassword literal"))
        return {"state": state, "summary": f"{flag_id} = {yn}", "notes": " ".join(notes_parts), "evidence": evidence}
    ...
    if flag_id == "has_release_minify_disabled":
        ...
        state = "fail" if has_feature else "pass"
        if has_feature:
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:gradle", "minifyEnabled", "minifyEnabled false in release"))
    ...
    if flag_id == "has_prevention_against_reverse_engineering":
        ...
        if has_minify:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "minifyEnabled", "minifyEnabled true in release"))
    ...
    if flag_id == "has_webview_remote_content":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "WebView.loadUrl", "loadUrl(http/https) detected"))
        yn = "YES" if has_feature else "NO"
        state = "fail" if has_feature else "pass"
    ...
    if flag_id == "has_runtime_global_kill_switch_for_security_incidents":
        ...
        if has_feature:
            for s in sources:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"ORG_INDEX:{s}", "kill_switch_keyword", "kill switch keyword detected"))
    ...
    if flag_id == "has_safe_mode_degraded_functionality_design":
        ...
        if has_feature:
            for s in sources:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"ORG_INDEX:{s}", "safe_mode_keyword", "safe mode keyword detected"))
    ...
    if flag_id == "has_controls_protecting_temporary_compiled_data":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "temp_protection", "temp/cache encrypted/protected keywords"))
    ...
    if flag_id == "has_temporary_compiled_data_securely_deleted":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "temp_cleanup", "cleanup/delete keywords"))
    ...
    if flag_id == "has_supports_manual_logout":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "logout()", "logout() method detected"))
        yn = "YES" if has_feature else "NO"
        state = "pass" if has_feature else "fail"
    ...
    if flag_id == "has_clears_local_session_data_on_logout":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "clearUserPreferencesData", "clears local prefs on logout"))
        yn = "YES" if has_feature else "NO"
        state = "pass" if has_feature else "fail"
    ...
    if flag_id == "has_clears_cookies_on_logout":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "cookie_clear", "removeAllCookies/clearCookies detected"))
        yn = "YES" if has_feature else "NO"
        state = "pass" if has_feature else "fail"
    ...
    if flag_id == "has_session_id_assigned_from_server_cookie":
        ...
        if has_feature:
            for p in paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "cookie_session", "CookieManager/JSESSIONID usage detected"))
        yn = "YES" if has_feature else "NO"
        state = "pass" if has_feature else "fail"
    ...
    if flag_id == "has_logout_invalidates_server_session":
        ...
        state = "pass" if has_feature else "fail"
        if has_feature:
            evidence.append(ev("SOURCE_CODE_OPENMRS", "openMRS.zip:code_scan", "/logout", "logout endpoint referenced"))
    ...
    if flag_id == "has_endpoint_requires_user_authentication":
        ...
        if has_basic_auth:
            for p in rest_paths:
                evidence.append(ev("SOURCE_CODE_OPENMRS", f"openMRS.zip:{p}", "Authorization: Basic", "RestServiceBuilder adds Authorization header"))
        yn = "YES" if has_feature else "NO"
        state = "pass" if has_feature else "fail"
        if has_feature:
...
PRIMARY_SOURCES_OVERRIDES = {
    ...
    "has_exported_broadcast_receivers_without_permission": ["SOURCE_CODE_OPENMRS"],
}
...
# raw openmrs scan hashes (deterministic)
openmrs_code_scan = {"files": {}}
for path, content in openmrs_texts.items():
...
for group in groups:
    ...
    for flag_id in group.get("flags", []):
        ...
        primary_sources = PRIMARY_SOURCES_OVERRIDES.get(flag_id, ["MobSF_STATIC", "SOURCE_CODE_OPENMRS"])
...
vision360_output["raw"]["openmrs_code_scan"] = openmrs_code_scan
